...
import { setCurrentSongData, setCurrentSongs, setPlaying, setPosition, StaticType, updateCurrentSongs } from '@/stores/Main';
...
export const setPositionThunk = (sound: Song): AppThunk => (dispatch, getState) => {
    ...
    for (const playlist of state.playlist) {
        ...
        fetchNui<number>('getCurrentSongDuration').then(res => {
            dispatch(updateCurrentSongs(playlist.songs));
